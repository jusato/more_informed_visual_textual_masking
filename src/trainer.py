...
PATH_TO_LABELS = './data/oid_v4_label_map.pbtxt'
...
class Trainer(object):
    ...
    def get_classes_name_and_scores(self, boxes, classes, category_index, label_id_offset, curr_index, marker): 
        for i in range(boxes.shape[0]):
            if classes[i] in six.viewkeys(category_index):
                line = curr_index
                column = i
                class_name = category_index[classes[i] + label_id_offset]['name']
                if class_name == 'Man' or class_name == 'Woman' or class_name == 'Girl' or class_name == 'Boy':
                    marker[line][column] = 1
    ...
    def mask_out_image(self, img_boxes, img_feats, img_labels):
        ...
        marker = torch.zeros(bs * n_regions)
        marker = marker.view(bs, n_regions)
        img_labels_np = np.array(img_labels)
        img_boxes_np = np.array(img_boxes)
        label_id_offset = 1
        for i in range(bs):
            self.get_classes_name_and_scores(img_boxes_np[i], img_labels_np[i], category_index, label_id_offset, i, marker)
        ...
        count = 0
        num_malefemale_nouns = 0 
        for i in range(bs):
            for j in range(n_regions):
                if marker[i][j] == 1: 
                    num_malefemale_nouns += 1
                if marker[i][j] == 1 and probs[i][j] > 0:
                    count += 1
        perc = count/(bs*n_regions)
        stack = []

        if perc < 0.025:
            new_perc = 0.025 - perc
            to_be_added = min(new_perc * (bs*n_regions), num_malefemale_nouns) 
            to_be_removed = to_be_added
            for i in range(bs):
                for j in range(n_regions):
                    if to_be_removed > 0 and probs[i][j] > 0 and marker[i][j] == 0:
                        stack.append(int(probs[i][j]))
                        probs[i][j] = 0
                        to_be_removed -= 1
                    elif to_be_added > 0 and probs[i][j] == 0 and marker[i][j] == 1:
                        if len(stack) == 0:
                            probs[i][j] = 1
                        else:
                            probs[i][j] = stack[len(stack)-1]
                            stack.pop()
                        to_be_added -= 1
                    if to_be_removed == 0 and to_be_added == 0:
                        break
