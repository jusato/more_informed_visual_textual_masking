...
onlyfiles1 = [f for f in listdir(FEAT_PATH) if isfile(join(FEAT_PATH, f))]
onlyfiles = []
...
# Tirar imagens corrompidas da lista de features para evitar _pickle.UnpicklingError: pickle data was truncated
print("Imagens Corrompidas:")
num_imgs_corr = 0
for i in range(0,len(onlyfiles1)):
  imgOK = True
  f_name = os.path.join(FEAT_PATH, onlyfiles1[i])
  with open(f_name, "rb") as f:
    try:
      ...
      print(onlyfiles1[i])
      imgOK = False
      num_imgs_corr = num_imgs_corr + 1

    if imgOK:
      onlyfiles.append(onlyfiles1[i])
print("Quantidade de imagens corrompidas =", num_imgs_corr)
